...
async function generateTweet() {
    try {
        ...
        const prompt = `
        あなたは優秀な広報担当エンジニアです。
        以下のコミットメッセージに基づいて、Twitter（X）に投稿するための魅力的なリリースツイートを作成してください。
        ...
        - 日本語で記述してください
        - 140文字以内に収めてください
        - 開発の進捗や新機能をアピールする、エキサイティングなトーンで
        - 適切な絵文字（🚀, ✨, 🛠️など）を1〜2個使用してください
        - ハッシュタグ #InfoAssistant #個人開発 を末尾に追加してください
        - URLやリポジトリへのリンクは含めないでください（後で付与します）
        - 出力はツイート本文のみにしてください（余計な説明は不要）
        `;
